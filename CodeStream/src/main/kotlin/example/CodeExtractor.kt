...
object CodeExtractor : CodeStream() {
  ...
  suspend fun invokeTopMovies(
    ...
    val entries =
      res.select("$hTag:matches((?i)$sTag.*(720p|1080p|2160p|4K))")
        .filter { element -> !element.text().contains("Batch/Zip", true) }.takeLast(2)
    entries.map { it ->
      val href =
      ...
      server.let { it ->
        ...
        app.get(link).document.select("a.btn").forEach {
          if (it.text().contains("Instant Download")) {
            ...
            callback.invoke(
              ExtractorLink(
                "TopMovies", "TopMovies $tagquality", trueurl, "", getQualityFromName(tags)
              )
            )
          }
          if (it.text().contains("Resume Cloud")) {
            ...
            callback.invoke(
              ExtractorLink(
                "TopMovies", "TopMoviesR $tagquality", resumelink, "", getQualityFromName(tags)
              )
            )
          } else {
  ...
  private suspend fun invokeModflix(
    ...
    val entries = res.select("div.thecontent $hTag:matches((?i)$sTag.*(720p|1080p|2160p))")
    entries.apmap { it ->
      val tags =
  ...
  private fun getUrlAndNames(urlArray: List<SmashyUrlArray>): List<Pair<String, String>> {
  ...
  suspend fun invokeBollyflix(
    ...
    entries.map {
      ...
      val encodedurl =
        app.get("https://web.sidexfee.com/?id=$token").text.substringAfter("link\":\"").substringBefore("\"};")
